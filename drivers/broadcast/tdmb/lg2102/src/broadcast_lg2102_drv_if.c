...
#define LG2102_USES_STATIC_BUFFER
...
#ifdef LG2102_USES_STATIC_BUFFER
...
int broadcast_drv_if_power_on(void)
{
	...
	if(gpMPI_Buffer == NULL)
	{
#ifndef LG2102_USES_STATIC_BUFFER
	...
	//tunerbb_drv_lg2102_set_userstop( 1 );
...
int broadcast_drv_if_power_off(void)
{
	...
	//tunerbb_drv_lg2102_set_userstop( 0 );
...
int broadcast_drv_if_get_ch_info(char* buffer, unsigned int* buffer_size)
{
	...
	retval = tunerbb_drv_lg2102_get_fic(buffer, buffer_size);
...
{
	tunerbb_drv_lg2102_set_userstop( mode );
	return OK ;
}
