...
void rtvRF_ConfigurePowerType(E_RTV_TV_MODE_TYPE eTvMode)
{
	...
	{
#ifdef RTV_ISDBT_ENABLE
		case RTV_TV_MODE_1SEG : REG54 = 0x1C; break;
#endif

#ifdef RTV_FM_ENABLE
		case RTV_TV_MODE_FM :   REG54 = 0x0C; break;
#endif

#ifdef RTV_TDMB_ENABLE
		case RTV_TV_MODE_TDMB : REG54 = 0x08; break;
#endif

#ifdef RTV_DAB_ENABLE
		case RTV_TV_MODE_DAB :  REG54 = 0x08; break;
#endif
		default: return;
	}
...
INT rtvRF_ConfigureAdcClock(E_RTV_TV_MODE_TYPE eTvMode, E_RTV_ADC_CLK_FREQ_TYPE eAdcClkFreqType)
{
	...
	{
#ifdef RTV_ISDBT_ENABLE
		case RTV_TV_MODE_1SEG : REGE9 = (0xD4 & 0xF0); break;
#endif		

#ifdef RTV_FM_ENABLE
		case RTV_TV_MODE_FM : REGE9 = (0x54 & 0xF0); break;
#endif

#ifdef RTV_TDMB_ENABLE
		case RTV_TV_MODE_TDMB : REGE9 = (0x54 & 0xF0); break;
#endif

#ifdef RTV_DAB_ENABLE
		case RTV_TV_MODE_DAB : REGE9 = (0x54 & 0xF0); break;
#endif
		default: return RTV_INVAILD_TV_MODE;
	}
	...
	for(i=0; i<10;i++)
	{
		...
		{
			RTV_DBGMSG0("[rtvRF_ConfigureAdcClock] CLOCK SYNTH 1st step  UnLock..\n");
		}
	...
	g_aeRtvAdcClkFreqType[RaonTvChipIdx] = eAdcClkFreqType;

	//RTV_DBGMSG1("[rtvRF_ConfigureAdcClock] ADC clk Type: %d\n", g_aeRtvAdcClkFreqType[RaonTvChipIdx]);
...
{	
	...
	if(eAdcClkFreqType == g_aeRtvAdcClkFreqType[RaonTvChipIdx])
		return RTV_SUCCESS;
	...
	for(i=0; i<10;i++)
	{
		...
		{
			RTV_DBGMSG0("[rtvRF_ChangeAdcClock] CLOCK SYNTH 1st step  UnLock..\n");
		}
	...
	{
		RTV_DBGMSG0("[rtvRF_ChangeAdcClock] DCDC CLOCK SYNTH  UnLock..\n");
	}
#endif
...
INT rtvRF_SetFrequency(E_RTV_TV_MODE_TYPE eTvMode, UINT nChNum, U32 dwChFreqKHz)
{
	...
	switch( eTvMode )
	{
#ifdef RTV_ISDBT_ENABLE
		case RTV_TV_MODE_1SEG :	
			if((g_eRtvCountryBandType==RTV_COUNTRY_BAND_BRAZIL) || (g_eRtvCountryBandType==RTV_COUNTRY_BAND_ARGENTINA))		
				nChNum -= 1;
			...
			switch( nChNum )
			{
				...
				case 30:  case 61:
					...
					break;
				default: 
					eAdcClkFreqType = g_aeAdcClkTypeTbl_ISDBT[0]; // 62ch => 8MHz @ 32MHz src clk.
					break;
			}	
		...
		case RTV_TV_MODE_FM :							
			...
			eAdcClkFreqType = g_aeRtvAdcClkFreqType[RaonTvChipIdx]; /* Only init adc clock type. */
			break;
#endif
	...
	g_afRtvChannelChange[RaonTvChipIdx] = TRUE; 
	...
	switch( eTvMode )
	{
		...
		case RTV_TV_MODE_1SEG :			
			...
			do{
				...
				{
					if((RTV_REG_GET(0x15) & 0x02) == 0x02) // PLL lock ?				
						break;
					else
					{
						RTV_REG_SET(0x20, nChNum);	
...
INT rtvRF_Initilize(E_RTV_TV_MODE_TYPE eTvMode)
{
	...
	if(eTvMode == RTV_TV_MODE_1SEG)
	{
		...
		if(RTV_REG_GET(0x01) == 0x09) // MTV220 ?
		{
    ...
    if(((U16)(RTV_REG_GET(0x10) << 8) | (U16)(RTV_REG_GET(0x11))) != 0xFFFF)
    {
		switch( eTvMode )
		{
#ifdef RTV_ISDBT_ENABLE
