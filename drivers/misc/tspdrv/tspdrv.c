...
static struct miscdevice brr_misc =
{
	.minor 	=	MISC_DYNAMIC_MINOR,
	.name 	=   BRR_NAME,
	.fops 	=   &brr_fop
...
static int suspend( struct platform_device *pdev, pm_message_t state ) 
{
	...
	if ( g_bIsPlaying ) {
    		...
    		vib_set_power_rail( g_vib->vdd_id, NV_FALSE);
...
{	
	...
	vib_set_power_rail( g_vib->vdd_id, NV_TRUE);
...
static int remove( struct platform_device *pdev)
{
	DbgOut( ( KERN_INFO "tspdrv: remove.\n" ) );
	return 0;
}
#else
static void platform_release( struct device *dev ) 
