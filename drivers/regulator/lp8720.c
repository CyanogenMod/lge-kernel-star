...
/*
	if(regulator_is_enabled(regulator_vddio_vi))
	{
		printk(KERN_DEBUG "%s: Trun off regulator_vddio_vi\n", __FUNCTION__, cam, af, vt);
		regulator_disable(regulator_vddio_vi);
	}
*/
	return	0;
}
...
/*
	if(!regulator_is_enabled(regulator_vddio_vi))
	{	
		printk(KERN_DEBUG "%s: Trun on regulator_vddio_vi\n", __FUNCTION__, cam, af, vt);
