 ...
 */
#ifdef  IFX_SPI_TX_DEBUG_MODE	 
 int ifx_check_data = 0;
int isr_count = 1;
#endif
...
{	
...
#ifdef  IFX_SPI_TX_DEBUG_MODE	
	IFX_SPI_TX_DEBUG("spi_data sending.. \n"); 
	ifx_check_data = 1;
	isr_count--;
#endif	
	wait_for_completion_timeout(&spi_data->ifx_read_write_completion,  msecs_to_jiffies(500));  //3ms -> 500ms

#ifdef  IFX_SPI_TX_DEBUG_MODE	
	IFX_SPI_TX_DEBUG("spi_data->ifx_ret_count =%d spi_data->ifx_spi_count =%d\n",spi_data->ifx_ret_count ,spi_data->ifx_spi_count); 
	ifx_check_data = 0;
#endif
	...
	{	
		...
		IFX_SPI_PRINTK("spi tx timeout!! SRDY : %d",pin_val);
...
static unsigned int ifx_spi_sync_read_write(struct ifx_spi_data *spi_data, unsigned int len)
{
	...
	{
		IFX_SPI_PRINTK("\t Transmission UNsuccessful, status = %d (ex.. -EIO= -5)", status);
        }
...
static irqreturn_t ifx_spi_handle_srdy_irq(int irq, void *handle)
{
	...
	if(ifx_check_data ==1)
	{
		...
		if(isr_count >=2)
		{
			IFX_SPI_TX_DEBUG("kek 2 times interrupt ...kek...kek  \n");	
		}
	}
#endif
#if 0	  
	if(spi_data && spi_data->ifx_tty)	//add to prevent the irq of srdy until spi opening
	{
		...
		wake_lock_timeout(&ifx_gspi_data->wake_lock, 5*HZ);	//5,, Unexpected interrupt or power consumption
#endif	
...
#if 0//ndef LJH_TEST
extern struct mutex cpu_dma_mutex;
#endif
...
static void ifx_spi_handle_work(struct work_struct *work)
{
	...
	else
	{
		...
		if(ifx_check_data ==1)
		{
			IFX_SPI_TX_DEBUG("spi_data receive\n");		
			ifx_check_data = 0;
		}
#endif		
		/* Once data transmission is completed, the MRDY signal is lowered */
