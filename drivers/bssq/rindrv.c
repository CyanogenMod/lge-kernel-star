...
#if 1 // wooje.shim@lge.com 2011.07.25 correct for wrong ip header
...
#else
unsigned char rawip_buffer[1511]; // ts0710.h DEF_TS0710_MTU 임 
#endif
...
static struct rin_st *rin_alloc(dev_t line)
{
	...
	if (!dev) {
		char name[IFNAMSIZ];
//20110531, yasong : ya.song@lge.com, LGE_RAW_IP LGE_KDDI_S
		//sprintf(name, "ppp%d", i);
		sprintf(name, "vsnet%d", i);
//20110531, yasong : ya.song@lge.com, LGE_RAW_IP LGE_KDDI_E
