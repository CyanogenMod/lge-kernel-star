...
static unsigned char	rwbuf[200] = {0,};
...
EXPORT_SYMBOL(magnetic_input_report);
...
/* jay.sim 2010.08.31 modification for modularization */
atomic_t mag_x, mag_y, mag_z;
EXPORT_SYMBOL(mag_x);
EXPORT_SYMBOL(mag_y);
EXPORT_SYMBOL(mag_z);

atomic_t compass_delay;
EXPORT_SYMBOL(compass_delay);
atomic_t compass_flag;
EXPORT_SYMBOL(compass_flag);
extern atomic_t compass_init;
...
int get_compass_flag(void)
{
	return atomic_read(&compass_flag);
}
EXPORT_SYMBOL(get_compass_flag);
...
static int AMI304_ReadSensorData(char *buf, int bufsize)
{
	...
	databuf[0] = AMI304_CTRL3_FORCE_BIT;
	NvCompassI2CSetRegs(compass_dev->hOdmComp, AMI304_REG_CTRL3, &databuf[0], 1);
...
#if 1
ssize_t store_motion_compass_delay(struct device *dev,  struct device_attribute *attr,const char *buf, size_t count)
{
	u32    val;
	...
	//	lprintk(D_SENSOR,"[%s] flag [%d] current_delay[%ld]\n", __func__, val, current_delay);
	printk("[%s] flag [%d] current_delay[%ld]\n", __func__, val, current_delay);
...
ssize_t show_motion_compass_delay(struct device *dev,  struct device_attribute *attr,  char *buf)
{
	u32    val;
	val = atomic_read(&compass_delay);
	return sprintf(buf, "%d\n",val);
}
#endif

static ssize_t show_motion_compass_status(struct device *dev,  struct device_attribute *attr,  char *buf)
{
	u32    val;
...
static ssize_t store_motion_compass_status(struct device *dev,  struct device_attribute *attr,  const char *buf, size_t count)
{
	u32    val;
	val = simple_strtoul(buf, NULL, 10);
...
static DEVICE_ATTR(compass_onoff, 0666, show_motion_compass_status, store_motion_compass_status);
static DEVICE_ATTR(compass_delay, 0666, show_motion_compass_delay, store_motion_compass_delay);
...
static int ami304daemon_open(struct inode *inode, struct file *file)
{
	...
	if (atomic_cmpxchg(&daemon_open_count, 0, 1)==0) {
		printk(KERN_INFO "Open device node:compass_daemon\n");
		ret = nonseekable_open(inode, file);
	}
	printk(KERN_INFO "%s:%d compass_daemon opened\n", __func__, __LINE__);
	return ret;
}
...
//			AMI304_Report_Value(en_dis_Report);
		...
		/* jay.sim : TODO have to move apropriate location */
		case AMI304_IOCTL_I2C_READ:
			data = (void __user *) arg;
			if (copy_from_user(&rwbuf, data, sizeof(rwbuf))) {
				printk("[%s:%d copy_from_user - Fail(AMI304_IOCTL_I2C_READ)\n", __func__, __LINE__);
				return -EFAULT;
			}

//			if (rwbuf[0] == 0x0E) {
				if (compassI2CGetRegs(rwbuf[2], &rwbuf[3], rwbuf[1]) == NV_FALSE) {
					printk("AMI304_IOCTL_I2C_READ : Address [%d], offset[%d] value[%d] len[%d] \n", rwbuf[0], rwbuf[2], rwbuf[3], rwbuf[1]);
					return -EINVAL;
				}

				if (copy_to_user(data, &rwbuf, sizeof(rwbuf))) {
					printk("[%s:%d copy_to_user - Fail(AMI304_IOCTL_I2C_READ)\n", __func__, __LINE__);
					return -EFAULT;
				}
//			}
			break;

		case AMI304_IOCTL_I2C_WRITE:
			data = (void __user *) arg;
			if (copy_from_user(&rwbuf, data, sizeof(rwbuf))) {
				printk("[%s:%d copy_from_user - Fail(AMI304_IOCTL_I2C_WRITE)\n", __func__, __LINE__);
				return -EFAULT;
			}

//			if (rwbuf[0] == 0x0E) {
				if (compassI2CSetRegs(rwbuf[2], &rwbuf[3], rwbuf[1] - 1) == NV_FALSE) {
					printk("AMI304_IOCTL_I2C_WRITE : Address [%d], offset[%d] value[%d] len[%d] \n", rwbuf[0], rwbuf[2], rwbuf[3], rwbuf[1]);
					return -EINVAL;
				}
//			}
			break;
...
#if 1
		case AMI304_IOCTL_MAGNETIC_RAW: /* wkkim add to recieve compass raw value */
			data = (void __user *) arg;
			if (copy_from_user(controlbuf, data, sizeof(int) * 3)) {
				printk("[%s:%d copy_from_user - Fail(AMI304_IOCTL_MAGNETIC_RAW)\n", __func__, __LINE__);
				return -EFAULT;
			}
			atomic_set(&mag_x, controlbuf[0]);
			atomic_set(&mag_y, controlbuf[1]);
			atomic_set(&mag_z, controlbuf[2]);
			break;
#endif
			/**/
...
int AMI304_Reset_Init(void)
{
	...
	databuf = 0x80;
	NvCompassI2CSetRegs(compass_dev->hOdmComp, AMI304_REG_CTRL3, &databuf, 1);
...
#if DEBUG_COMPASS
	printk("[%s:%d] Compass Sensor Init\n", __FUNCTION__, __LINE__);
#endif
...
#if DEBUG_COMPASS
	printk("[%s:%d] AMI304_REG_CTRL1(R) = 0x%x \n", __FUNCTION__, __LINE__, ctrl1);
	printk("[%s:%d] AMI304_REG_CTRL2(R) = 0x%x \n", __FUNCTION__, __LINE__, ctrl2);
	printk("[%s:%d] AMI304_REG_CTRL3(R) = 0x%x \n", __FUNCTION__, __LINE__, ctrl3);
#endif

	if (mode == AMI304_FORCE_MODE) {
		databuf = ctrl1 | AMI304_CTRL1_PC1 | AMI304_CTRL1_FS1_FORCE;
		write_lock(&ami304_data.lock);
		ami304_data.mode = AMI304_FORCE_MODE;
		write_unlock(&ami304_data.lock);
	} else	{
		databuf = ctrl1 | AMI304_CTRL1_PC1 | AMI304_CTRL1_FS1_NORMAL | AMI304_CTRL1_ODR1;
		write_lock(&ami304_data.lock);
		ami304_data.mode = AMI304_NORMAL_MODE;
		write_unlock(&ami304_data.lock);
	}
	NvCompassI2CSetRegs(compass_dev->hOdmComp, AMI304_REG_CTRL1, &databuf, 1);
#if DEBUG_COMPASS
	printk("[%s:%d] AMI304_REG_CTRL1(W) = 0x%x -> 0x%x \n", __FUNCTION__, __LINE__, ctrl1, databuf);
#endif

	databuf = ctrl2 | AMI304_CTRL2_DREN;
	NvCompassI2CSetRegs(compass_dev->hOdmComp, AMI304_REG_CTRL2, &databuf, 1);
#if DEBUG_COMPASS
	printk("[%s:%d] AMI304_REG_CTRL2(W) = 0x%x -> 0x%x \n", __FUNCTION__, __LINE__, ctrl2, databuf);
#endif

	/* TODO : check AMI304_CTRL3_B0_LO_CLR value */
	databuf = ctrl3 | AMI304_CTRL3_B0_LO_CLR;
	NvCompassI2CSetRegs(compass_dev->hOdmComp, AMI304_REG_CTRL3, &databuf, 1);
#if DEBUG_COMPASS
	printk("[%s:%d] AMI304_REG_CTRL3(W) = 0x%x -> 0x%x \n", __FUNCTION__, __LINE__, ctrl3, databuf);
#endif
...
static enum hrtimer_restart motion_compass_timer_func(struct hrtimer *timer)
{
	...
	if (atomic_read(&compass_flag)) {
		queue_work(compass_dev->compass_wq, &compass_dev->compass_work);

		polling_time = atomic_read(&compass_delay);
		hrtimer_start(&compass_dev->timer, ktime_set(0, polling_time * 1000000), HRTIMER_MODE_REL); /* start now */
	}
...
#if DEBUG_COMPASS
	printk("[%s:%d] Compass Sensor Start\n", __FUNCTION__, __LINE__);
#endif
	compass_dev = kzalloc(sizeof(*compass_dev), GFP_KERNEL);
	if (compass_dev == NULL) {
		err = -ENOMEM;
...
//	compass_dev = compass;
	platform_set_drvdata(pdev, compass_dev);
#if DEBUG_COMPASS
	printk("[%s:%d] Allocate Memory for Compass Device(%lu)\n", __FUNCTION__, __LINE__);
#endif

	/* Setting I2C & GPIO, Power on */
...
#if DEBUG_COMPASS
	printk("[%s:%d] Open NVOdm for Compass Device(%lu)\n", __FUNCTION__, __LINE__);
#endif

	/* On soft reset : Perform to same routine as POR */
	AMI304_Reset_Init();

	AMI304_Init(AMI304_FORCE_MODE); // default is Force State
#if DEBUG_COMPASS
	printk("[%s:%d] Compass Sensor Soft-reset(%lu) \n", __FUNCTION__, __LINE__);
#endif

	compass_dev->input_dev = input_allocate_device();
	if (compass_dev->input_dev == NULL) {
		err = -ENOMEM;
...
//	compass_dev->input_dev = input_dev;
	...
	err = input_register_device(compass_dev->input_dev);
	if (err) {
		pr_err("tegra_compass_probe: Unable to register %s\
				input device\n", compass_dev->input_dev->name);
		goto register_input_dev_fail;
	}

#if DEBUG_COMPASS
	printk("[%s:%d] Register Input Device for Compass Sensor(%lu) \n", __FUNCTION__, __LINE__);
#endif

	err = sysfs_create_group(&dev->kobj, &ami304_attribute_group);

#if DEBUG_COMPASS
	printk("[%s:%d] Create sysfs attibutes for Compass Sensor(%lu) \n", __FUNCTION__, __LINE__);
#endif

	/* TODO : move to integrated module ? */
...
#if DEBUG_COMPASS
	printk("[%s:%d] Register Misc. Dev for 'ami304' device file(%lu) \n", __FUNCTION__, __LINE__);
#endif
...
#if DEBUG_COMPASS
	printk("[%s:%d] Register Misc. Dev for 'ami304daemon' device file(%lu) \n", __FUNCTION__, __LINE__);
#endif
	...
	atomic_set(&compass_init, 1);
	sensors_wake_up_now();
	printk("%s compass init done\n", __func__);
...
register_misc_fail1:
	sysfs_remove_group(&dev->kobj, &ami304_attribute_group);
...
static NvS32 ami304_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&(pdev->dev.kobj), &ami304_attribute_group);
	input_unregister_device(compass_dev->input_dev);
	input_free_device(compass_dev->input_dev);
	close_odm_compass();
	kfree(compass_dev);
...
static NvS32 __devinit ami304_init(void)
{
	...
	ret = platform_driver_register(&tegra_compass_driver);
