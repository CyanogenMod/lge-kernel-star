...
#define NVBATTERY_POLLING_INTERVAL 260 /* 60+200 seconds */ // Use with HZ(1sec) const
//20100929, byoungwoo.yoon@lge.com, RTC setting for checking battery status during sleep
#define SLEEP_BAT_CHECK_PERIOD 2060 /* 60 + 2000 seconds */
#define CRITICAL_BAT_CHECK_PERIOD 60 // second
#define CBC_REQUEST_TIME_FIRST	35000 // ms
#define CBC_REQUEST_TIME_SECOND	50000 // ms
#define CBC_REQUEST_TIME_NORMAL	180000 // ms
#define CBC_REQUEST_TIME_CRITICAL 480000 // ms
#define GAUGE_FOLLOW_TIME	160 // second
	...
	switch (batt_dev->charger_state_machine)
	{
		...
		case CHARGER_STATE_SHUTDOWN:
			if ( at_boot_state == 0 )
			{
				...
				LDC("[FACTORY] at_boot_state ==0, no effect in operation");
			break;
...
{
	if (batt_dev->Boot_TA_setting == NV_FALSE)
	{
	...
	else // batt_dev->Boot_TA_setting == NV_TRUE
	{
		charging_ic->status = (max8922_status)Mode;
	}
}
...
static void star_capacity_from_voltage_via_calculate(void)
{
	...
	if ((batt_dev->ACLineStatus == NV_TRUE) && (batt_dev->Boot_TA_setting == NV_FALSE))
	{
	...
	else if ((batt_dev->ACLineStatus == NV_FALSE) || (batt_dev->Boot_TA_setting == NV_TRUE))
	{
...
//20100926, jh.ahn@lge.com, Communication with BatteryService for AT Command [END]
		...
		{
			if ((cbc_value == 0) && (batt_dev->BatteryGauge != 0))
			{
				batt_dev->BatteryGauge = cbc_value;
			}
			else if ((cbc_value == 100) && (batt_dev->BatteryGauge == 0))
			{
				batt_dev->BatteryGauge = cbc_value;
			}
			else
			{
				...
				else if (cbc_value == 100)
				{
					if (((batt_dev->batt_vol > 4130) || (batt_dev->BatteryLifePercent >= 96)))
					{
						...
						batt_dev->BatteryGauge = 99;
						batt_dev->BatteryGauge_on = NV_TRUE;
					}
				}
				else
				{
					if (STAR_BAT_MAX(cbc_value, batt_dev->Capacity_Voltage) - STAR_BAT_MIN(cbc_value, batt_dev->Capacity_Voltage) <= 20)
					{
						batt_dev->BatteryLifePercent = cbc_value;
						batt_dev->BatteryGauge = cbc_value;
						batt_dev->BatteryGauge_on = NV_TRUE;
						}
					else
				...
				if ((batt_dev->Boot_TA_setting == NV_TRUE) && (batt_dev->BatteryGauge_on == NV_TRUE))
				{
					batt_dev->Boot_TA_setting = NV_FALSE;
					charging_ic_active(charging_ic->status);
				}
			}
		}
		...
		if (batt_dev->BatteryGauge_on == NV_TRUE)
		{
			...
			if (cbc_value == 0)
			{
				if (((batt_dev->batt_vol < 3400) || (batt_dev->BatteryLifePercent <= 3)))
				{
					batt_dev->BatteryLifePercent = 1;
					batt_dev->BatteryGauge = 1;
				}
				else if (((batt_dev->batt_vol < 3300) || (batt_dev->BatteryLifePercent <= 3)))
				{
			...
			{
				if ( cbc_value >= batt_dev->BatteryLifePercent ) //((batt_dev->ACLineStatus == NV_TRUE) && ((batt_dev->batt_health == POWER_SUPPLY_HEALTH_GOOD) || (batt_dev->batt_health == POWER_SUPPLY_HEALTH_OVERHEAT)))
				{
					if ((cbc_value - batt_dev->BatteryLifePercent) <= 3)
					{
						if ((cbc_value - batt_dev->BatteryLifePercent) <= 1)
						{
							batt_dev->BatteryLifePercent = cbc_value;
						}
						else if ((cbc_value - batt_dev->BatteryLifePercent) > 1)
						{
							star_gauge_follower_func();
						}
					}
					else if ((STAR_BAT_MAX(cbc_value, batt_dev->BatteryGauge) - STAR_BAT_MIN(cbc_value, batt_dev->BatteryGauge)) <= 1)
					{
						LDB("[Critical]: Unexpected CBC change.. Maybe First gauge CBC value or Check Gauge IC");
						if (STAR_BAT_MAX(cbc_value, batt_dev->Capacity_Voltage) - STAR_BAT_MIN(cbc_value, batt_dev->Capacity_Voltage) <= 5)
						{
							batt_dev->BatteryLifePercent = cbc_value;
							batt_dev->BatteryGauge = cbc_value;
							batt_dev->BatteryGauge_on = NV_TRUE;
					...
					{
						LDB("[Critical]: CBC value poping!!!");
					}
				}
				else if ( cbc_value < batt_dev->BatteryLifePercent ) // ((batt_dev->ACLineStatus == NV_FALSE) || ((batt_dev->batt_health != POWER_SUPPLY_HEALTH_GOOD) && (batt_dev->batt_health != POWER_SUPPLY_HEALTH_OVERHEAT)))
				{
					if ((batt_dev->BatteryLifePercent - cbc_value) <= 3)
					{
						if ((batt_dev->BatteryLifePercent == 100 ) && (batt_dev->charger_state_machine == CHARGER_STATE_FULLBATTERY))
						{
							batt_dev->BatteryLifePercent = 100;
						}
						else
						{
							if ((batt_dev->BatteryLifePercent - cbc_value) <= 1)
							{
								batt_dev->BatteryLifePercent = cbc_value;
							}
							else if ((batt_dev->BatteryLifePercent - cbc_value) > 1)
							{
								if (batt_dev->BatteryLifePercent <= 90)
									star_gauge_follower_func();
								if ((batt_dev->BatteryLifePercent <= 0) && (batt_dev->batt_vol > 3300))
									batt_dev->BatteryLifePercent = 1;
							}
						}
					}
...
//20100702, jh.ahn@lge.com, for Fuel gauge reset [END]
	...
	else if ((value < 0 || value > 100) && (value != 52407))
	{
		...
		batt_dev->BatteryLifePercent = 60; // default setting
		return count;
	}
//20100702, jh.ahn@lge.com, for Fuel gauge reset [END]
	return count;
		...
		if ((BatData.batteryVoltage != NVODM_BATTERY_DATA_UNKNOWN) && (BatData.batteryTemperature != NVODM_BATTERY_DATA_UNKNOWN))
		{
			...
			if ((1500 < BatData.batteryVoltage) && (BatData.batteryVoltage < 5000))
			{
				...
				else
				{
					...
					if ((3450 < BatData.batteryVoltage) && (4050 > BatData.batteryVoltage))
...
static void star_battery_data_poll_period_change(void)
{
		...
		|| (batt_dev->charger_state_machine == CHARGER_STATE_FULLBATTERY))
	{
		...
		critical_period_wake = CRITICAL_BAT_CHECK_PERIOD;
	}
	else
	{
	...
	else // batt_dev->BatteryGauge_on == NV_TRUE
	{
		...
		else /* gauge follower run rapidly with TA/USB */
		{
			if (batt_dev->ACLineStatus == NV_TRUE)
			{
				gauge_period_data = (GAUGE_FOLLOW_TIME/2)*HZ;
				gauge_period_wake = GAUGE_FOLLOW_TIME;
			}
			else
			{
				gauge_period_data = (GAUGE_FOLLOW_TIME*2)*HZ;
				gauge_period_wake = GAUGE_FOLLOW_TIME*4;
			}
		}
	}
	...
	batt_dev->sleep_bat_check_period = STAR_BAT_MAX(CRITICAL_BAT_CHECK_PERIOD, (100*(NvU32)((batt_dev->sleep_bat_check_period + 40)/100) - 40));
	...
	//LDB("[bat_sysfs] polling_interval: Critical(%d, %d), Vol(%d, %d), Tem(%d, %d), Gauge(%d, %d)", critical_period_data, critical_period_wake, vol_period_data, vol_period_wake, temp_period_data, temp_period_wake, gauge_period_data, gauge_period_wake);
...
static void star_gauge_follower_func(void)
{
	...
	if (batt_dev->ACLineStatus == NV_TRUE)
	{
		if (batt_dev->BatteryLifePercent >= 98)
			gauge_follower_time_mask = (GAUGE_FOLLOW_TIME*4)*1000 - 20;
		else
			gauge_follower_time_mask = (GAUGE_FOLLOW_TIME/2)*1000 - 20;
	}
	else
	{
		if (batt_dev->BatteryLifePercent >= 98)
			gauge_follower_time_mask = (GAUGE_FOLLOW_TIME*6)*1000 - 20;
		else
			gauge_follower_time_mask = (GAUGE_FOLLOW_TIME*2)*1000 - 20;
	}

	gauge_follower_time = NvOsGetTimeMS();
	if ((gauge_follower_time - batt_dev->gauge_follow_time) >= gauge_follower_time_mask)
	{
		batt_dev->gauge_follow_time = gauge_follower_time;

		if (batt_dev->ACLineStatus == NV_TRUE)
		{
			if (batt_dev->BatteryGauge > batt_dev->BatteryLifePercent)
				batt_dev->BatteryLifePercent += 1;
			else if ((batt_dev->BatteryGauge < batt_dev->BatteryLifePercent) && (batt_dev->BatteryGauge >= 98))
				batt_dev->BatteryLifePercent += 1;

			if (batt_dev->BatteryLifePercent == 100)
			{
				batt_dev->BatteryLifePercent = 99;
				tegra_battery_gpio_read_timer_func(99);
			}
		}
...
//20100926, jh.ahn@lge.com, Communication with BatteryService for AT Command [START]
	star_battery_infomation_update();
	charger_control_with_battery_temp();
	mod_timer(&(batt_dev->charger_state_read_timer), jiffies + msecs_to_jiffies(250));
...
//20100824, jh.ahn@lge.com, get capacity using battery voltage for demo [START]
	...
	if (batt_dev->repeat_index == NV_TRUE)
	{
		...
		{
			batt_dev->BatteryLifePercent = 30;
...
//20100924, jh.ahn@lge.com, For Battery recharge [START]
	...
	if ((( batt_dev->BatteryGauge <= 97 ) || (batt_dev->batt_vol < 4135)) && (batt_dev->charger_state_machine == CHARGER_STATE_FULLBATTERY))
	{
...
{
	static NvU32 cbc_req_time = 0;

	if ((batt_dev->at_comm_ready == NV_FALSE) && (batt_dev->cbc_clear_state != Clear_No_Response_3))
	{
	...
	{
		if (batt_dev->cbc_request_state == Request_First_Time)
		{
			if ((batt_dev->RIL_ready == NV_TRUE) && (batt_dev->BatteryGauge_on == NV_FALSE))
			{
				batt_dev->cbc_request_time = CBC_REQUEST_TIME_SECOND - (batt_dev->cbc_clear_state*3000);
				LDB("[CBC]: First request with clear/time(%d,%d)", batt_dev->cbc_clear_state, batt_dev->cbc_request_time);
				batt_dev->cbc_request_state = Request_Second_Time;
				batt_dev->at_comm_want = AT_COMM_CBC;
				batt_dev->at_comm_ready = NV_TRUE;
				star_cp_at_comm_request();
			}
		}
		else if (batt_dev->cbc_request_state == Request_Second_Time)
		{
			if ((batt_dev->RIL_ready == NV_TRUE) && (batt_dev->BatteryGauge_on == NV_FALSE))
			{
				LDB("[CBC]: Second request with clear(%d)", batt_dev->cbc_clear_state);
				batt_dev->cbc_request_time = CBC_REQUEST_TIME_NORMAL;
				batt_dev->cbc_request_state = Request_Normal_Time;
				batt_dev->at_comm_want = AT_COMM_CBC;
				batt_dev->at_comm_ready = NV_TRUE;
				star_cp_at_comm_request();
			}
		...
		else
		{
			...
			if (batt_dev->cbc_request_state == Request_Normal_Time)
			{
				if ((cbc_req_time - batt_dev->last_cbc_time) > CBC_REQUEST_TIME_CRITICAL) // second
				{
					if ((batt_dev->RIL_ready == NV_TRUE) && (batt_dev->BatteryGauge_on == NV_FALSE))
					{
						LDB("[CBC]: NO CBC with too long time, execute CBC");
						batt_dev->at_comm_want = AT_COMM_NO;
						batt_dev->at_comm_ready = NV_TRUE;
						star_cp_at_comm_request();
					}
			...
			else if (batt_dev->cbc_request_state == Request_Resume_Time)
			{
				if ((cbc_req_time - batt_dev->last_cbc_time) > CRITICAL_BAT_CHECK_PERIOD*2) // second
				{
					if ((batt_dev->RIL_ready == NV_TRUE) && (batt_dev->BatteryGauge_on == NV_FALSE))
					{
						//LDB("[CBC]: Execute CBC when Resume time");
						batt_dev->cbc_request_state = Request_Normal_Time;
						batt_dev->at_comm_want = AT_COMM_NO;
						batt_dev->at_comm_ready = NV_TRUE;
						star_cp_at_comm_request();
		...
		mod_timer(&(batt_dev->battery_gauge_timer), jiffies + msecs_to_jiffies(batt_dev->cbc_request_time));
	}
}
...
{
	switch ((charge_ic_state_machine)batt_dev->charger_state_machine)
	{
			...
			switch ((charge_ic_status_transition)*chg_state_new)
			{
				...
				case CHGSB_PGB_OFF_OFF:
					...
					*chg_state_now = CHARGER_STATE_SHUTDOWN;
					break;
				case CHGSB_PGB_OFF_ON:
					LDC("[chg_state] STANDBY/OFF_ON/STANDBY");
					*chg_state_now = batt_dev->charger_state_machine;
					break;
				default:
					LDC("[Critical] Unknown Charger state GPIO!!!(CHGSB_PGB_ON_OFF)");
...
//20100609, jh.ahn@lge.com, Charger Code : GPIO Setting [END]
	...
	batt_dev->batt_health = POWER_SUPPLY_HEALTH_GOOD;
	batt_dev->charging_source = NvCharger_Type_AC; // Initially, Assume Charger setting is FACTORY_MODE.
	batt_dev->charging_enabled = NvCharge_Control_Charging_Enable; // Initially, Assume FACTORY_MODE
	batt_dev->BatteryLifePercent = 60; // default setting
	batt_dev->BatteryGauge = 40;
...
//20100630, jh.ahn@lge.com, Change probe function structure [END]
	...
	queue_delayed_work(batt_dev->battery_workqueue, &batt_dev->battery_status_poll_work, HZ*20);
	queue_delayed_work(batt_dev->battery_workqueue, &batt_dev->battery_id_poll_work, HZ*2);
...
static int tegra_battery_resume(struct platform_device *dev)
{
	...
	batt_dev->cbc_request_state = Request_Resume_Time;
	batt_dev->at_comm_want = AT_COMM_NO;
	batt_dev->at_comm_ready = NV_TRUE;

	setup_timer(&(batt_dev->battery_gauge_timer), star_cbc_gauge_request_timer_func, 0);
	mod_timer(&(batt_dev->battery_gauge_timer), jiffies + msecs_to_jiffies(200));
