...
static int __init tegra_pinmux_init_tristate_refcount(void)
{
	int i;
	for (i = TEGRA_PINGROUP_ATA; i < TEGRA_MAX_PINGROUP; ++i) {
		tristate_refcount[i] = tegra_pinmux_get_tristate(i);
	}
